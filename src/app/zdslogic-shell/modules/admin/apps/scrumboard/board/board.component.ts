...
import { ScrumboardService } from 'app/zdslogic-shell/modules/admin/apps/scrumboard/scrumboard.service';
import { Board, Card, List } from 'app/zdslogic-shell/modules/admin/apps/scrumboard/scrumboard.models';
...
{
    board: Board;
    listTitleForm: UntypedFormGroup = new UntypedFormGroup({
    ...
    private _calculatePositions(event: CdkDragDrop<any[]>): any[]
    {
        ...
        if ( !Number.isInteger(currentItem.position) || currentItem.position >= this._maxPosition )
        {
            ...
            items = items.map((value, index): any {
