...
export class SocketClientOneService implements OnDestroy {
  ...
  subscribe(destination: string, id: string): Observable<any> {
    ...
    return new Observable<any>((observer) => {
      const subscription: StompSubscription = client.subscribe(destination, (message) => {
        observer.next(SocketClientOneService.jsonHandler(message));
      }, { id });
    });
  }

  onMessage(id: string, topic: string, handler = SocketClientOneService.jsonHandler): Observable<any> {
  ...
  onPlainMessage(id: string, topic: string): Observable<string> {
    return this.onMessage(id, topic, SocketClientOneService.textHandler);
  }
