...
export class SocketClientElizaService implements OnDestroy {
  ...
  subscribe(destination: string, id: string): Observable<any> {
    ...
    return new Observable<any>((observer) => {
      const subscription: StompSubscription = client.subscribe(destination, (message) => {
        observer.next(SocketClientElizaService.textHandler(message));
      }, { id });
    });
  }

  onMessage(id: string, topic: string, handler = SocketClientElizaService.jsonHandler): Observable<any> {
  ...
  onPlainMessage(id: string, topic: string): Observable<string> {
    return this.onMessage(id, topic, SocketClientElizaService.textHandler);
  }
