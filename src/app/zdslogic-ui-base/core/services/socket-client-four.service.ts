...
export class SocketClientFourService implements OnDestroy {
  ...
  connect(): Observable<CompatClient> {
    ...
    return new Observable<CompatClient>((observer) => {
      this.state.pipe(filter(state => state === SocketClientState.CONNECTED)).subscribe(() => {
        this._dataSharingService.isActiveContactsReady.next(true);
  ...
  subscribe(destination: string, id: string): Observable<any> {
    ...
    return new Observable<any>((observer) => {
      const subscription: StompSubscription = client.subscribe(destination, (message) => {
        observer.next(SocketClientFourService.jsonHandler(message));
      }, { id });
    });
  }

  onMessage(id: string, topic: string, handler = SocketClientFourService.jsonHandler): Observable<any> {
  ...
  onPlainMessage(id: string, topic: string): Observable<string> {
    return this.onMessage(id, topic, SocketClientFourService.textHandler);
  }
