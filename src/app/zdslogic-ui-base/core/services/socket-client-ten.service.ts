...
export class SocketClientTenService implements OnDestroy {
	...
	subscribe(destination: string, id: string): Observable<any> {
		...
		return new Observable<any>((observer) => {
			const subscription: StompSubscription = client.subscribe(destination, (message) => {
				observer.next(SocketClientTenService.jsonHandler(message));
			}, { id });
		});
	}

	onMessage(id: string, topic: string, handler = SocketClientTenService.jsonHandler): Observable<any> {
	...
	onPlainMessage(id: string, topic: string): Observable<string> {
		return this.onMessage(id, topic, SocketClientTenService.textHandler);
	}
