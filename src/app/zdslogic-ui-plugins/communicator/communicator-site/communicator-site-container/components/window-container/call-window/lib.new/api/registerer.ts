...
export class Registerer {
  ...
  public constructor(userAgent: UserAgent, options: RegistererOptions = {}) {
    ...
    // initialize logger
    this._angularLogService = userAgent.getLogger("sip.Registerer");
  ...
  public dispose(): Promise<void> {
    ...
    this.disposed = true;
    this._angularLogService.log(`Registerer ${this.id} in state ${this.state} is being disposed`);
  ...
  public register(options: RegistererRegisterOptions = {}): Promise<OutgoingRegisterRequest> {
    ...
    const outgoingRegisterRequest = this.userAgent.userAgentCore.register(this.request, {
      onAccept: (response): void => {
        ...
        if (!contacts) {
          this._angularLogService.error("No Contact header in response to REGISTER, dropping response.");
          this.unregistered();
          return;
        }
        ...
        if (contact === undefined) {
          this._angularLogService.error("No Contact header pointing to us, dropping response");
          this.unregistered();
        ...
        if (expires === undefined) {
          this._angularLogService.error("Contact pointing to us is missing expires parameter, dropping response");
          this.unregistered();
          this.waitingToggle(false);
          return;
        }
      ...
      onRedirect: (response): void => {
        this._angularLogService.error("Redirect received. Not supported.");
        this.unregistered();
      ...
      onReject: (response): void => {
        if (response.message.statusCode === 423) {
          ...
          if (!response.message.hasHeader("min-expires")) {
            ...
            this._angularLogService.error("423 response received for REGISTER without Min-Expires, dropping response");
            this.unregistered();
            this.waitingToggle(false);
            return;
          }
        ...
        this._angularLogService.warn(`Failed to register, status code ${response.message.statusCode}`);
  ...
  public unregister(options: RegistererUnregisterOptions = {}): Promise<OutgoingRegisterRequest> {
    ...
    if (this._state !== RegistererState.Registered && !options.all) {
      this._angularLogService.warn("Not currently registered, but sending an unregister anyway.");
    }
    ...
    const outgoingRegisterRequest = this.userAgent.userAgentCore.register(this.request, {
      ...
      onRedirect: (response): void => {
        this._angularLogService.error("Unregister redirected. Not currently supported.");
        this.unregistered();
        if (options.requestDelegate && options.requestDelegate.onRedirect) {
          options.requestDelegate.onRedirect(response);
        }
      ...
      onReject: (response): void => {
        this._angularLogService.error(`Unregister rejected with status code ${response.message.statusCode}`);
        this.unregistered();
        if (options.requestDelegate && options.requestDelegate.onReject) {
          options.requestDelegate.onReject(response);
        }
  ...
  private registered(expires: number): void {
    ...
    this.registrationExpiredTimer = setTimeout(() => {
      this._angularLogService.warn("Registration expired");
      this.unregistered();
    }, expires * 1000);
  ...
  private stateTransition(newState: RegistererState): void {
    ...
    this._angularLogService.log(`Registration transitioned to state ${this._state}`);
  ...
  private waitingToggle(waiting: boolean): void {
    ...
    this._angularLogService.log(`Waiting toggled to ${this._waiting}`);
  ...
  private waitingWarning(): void {
    ...
    this._angularLogService.warn(message);
  }
