...
export class IncomingMessage {
  ...
  public parseHeader(name: string, idx = 0): any | undefined {
    ...
    if (!this.headers[name]) {
      // this._angularLogService.log("header '" + name + "' not present");
      return;
    } else if (idx >= this.headers[name].length) {
      // this._angularLogService.log("not so many '" + name + "' headers present");
      return;
    }
    ...
    if (parsed === -1) {
      ...
      // this._angularLogService.warn('error parsing "' + name + '" header field with value "' + value + '"');
      return;
    } else {
