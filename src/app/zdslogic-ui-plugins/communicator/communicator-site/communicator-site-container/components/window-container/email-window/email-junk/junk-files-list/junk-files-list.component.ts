...
import { JunkFileDeleteDialogComponent } from './../junk-file-delete/junk-file-delete-dialog.component';
import { EMailJunkFile } from 'app/zdslogic-ui-plugins/emails/core/models/email-junk-file.model';
import { EMailFilesService } from 'app/zdslogic-ui-plugins/emails/core/services/email-files.service';

@Component({
	selector: 'app-junk-files-list',
	templateUrl: './junk-files-list.component.html',
	styleUrls: ['./junk-files-list.component.scss']
})
export class JunkFilesListComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	public onMailSelected(mail: EMailJunkFile): void {
		const url = `/my-emails/junk/${mail.id}`;
		this._router.navigate([url]);
	};

	public redirectToDetails = (id: string): void => {
		const url = `/my-emails/junk/${id}`;
		this._router.navigate([url]);
	};
	...
	public emptyJunk(): void {

		//const id: string = this._activeRoute.snapshot.params['id'];
		const apiUrl = 'my-junk-emails/empty';
