...
import { EMailBlacklistFileAttachment } from 'app/zdslogic-ui-plugins/emails/core/models/email-blacklist-file-attachment.model';
import { EMailContactSend } from 'app/zdslogic-ui-plugins/emails/core/models/emailContactSend.model';

@Component({
	selector: 'app-blacklist-file-details',
	templateUrl: './blacklist-file-details.component.html',
	styleUrls: ['./blacklist-file-details.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class BlacklistFileDetailsComponent implements OnInit, OnDestroy {
	...
	public mail: EMailBlacklistFile;
	public showAccounts;
	...
	private getFileDetails = (): void => {
		...
		const apiUrl = `blacklist/${this.emailId}`;
		this._repository.getData(apiUrl)
			.pipe(
				takeUntil(this._unsubscribeAll),
				mergeMap((result) => {
					const mails = result as EMailBlacklistFile;
					if (mails?.toId) {
				...
				this.mail = result as EMailBlacklistFile;
			},
				(error) => {
	...
	public redirectToDownload = (id: string): void => {
		...
		this._repository.getData(apiUrl)
			...
			.subscribe((result) => {
				const file = result as EMailBlacklistFileAttachment;
	...
	moveToDelete(): void {
		this.emailFilesService.moveBlacklistToDelete(this.emailId);
	}
}
