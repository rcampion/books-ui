...
export class InviteServerTransaction extends ServerTransaction {
  ...
  private startProgressExtensionTimer(): void {
    ...
    if (this.progressExtensionTimer === undefined) {
      this.progressExtensionTimer = setInterval(() => {
        this._angularLogService.debug(`Progress extension timer expired for INVITE server transaction ${this.id}.`);
        if (!this.lastProvisionalResponse) {
   ...
   */
  private timerH(): void {
    this._angularLogService.debug(`Timer H expired for INVITE server transaction ${this.id}.`);
    if (this.state === TransactionState.Completed) {
      this._angularLogService.warn("ACK to negative final response was never received, terminating transaction.");
   ...
   */
  private timerI(): void {
    this._angularLogService.debug(`Timer I expired for INVITE server transaction ${this.id}.`);
    this.stateTransition(TransactionState.Terminated);
  }
   ...
   */
  private timerL(): void {
    this._angularLogService.debug(`Timer L expired for INVITE server transaction ${this.id}.`);
