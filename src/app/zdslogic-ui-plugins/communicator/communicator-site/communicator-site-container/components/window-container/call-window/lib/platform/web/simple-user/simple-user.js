...
export class SimpleUser {
    ...
    constructor(server, options = {}) {
        ...
        this.userAgent.delegate = {
            ...
            onConnect: () => {
                this._angularLogService.log(`[${this.id}] Connected`);
                if (this.delegate && this.delegate.onServerConnect) {
                    this.delegate.onServerConnect();
                }
                if (this.registerer && this.registerRequested) {
                    this._angularLogService.log(`[${this.id}] Registering...`);
                    this.registerer.register().catch((e) => {
                        this._angularLogService.error(`[${this.id}] Error occurred registering after connection with server was obtained.`);
                        this._angularLogService.error(e.toString());
                    });
                }
            },
            // Handle connection with server lost
            onDisconnect: (error) => {
                this._angularLogService.log(`[${this.id}] Disconnected`);
                if (this.delegate && this.delegate.onServerDisconnect) {
                    this.delegate.onServerDisconnect(error);
                }
                if (this.session) {
                    this._angularLogService.log(`[${this.id}] Hanging up...`);
                    this.hangup() // cleanup hung calls
                        .catch((e) => {
                        this._angularLogService.error(`[${this.id}] Error occurred hanging up call after connection with server was lost.`);
                        this._angularLogService.error(e.toString());
                    });
                }
                if (this.registerer) {
                    this._angularLogService.log(`[${this.id}] Unregistering...`);
                    this.registerer
                        .unregister() // cleanup invalid registrations
                        .catch((e) => {
                        this._angularLogService.error(`[${this.id}] Error occurred unregistering after connection with server was lost.`);
                        this._angularLogService.error(e.toString());
                    });
                }
            ...
            onInvite: (invitation) => {
                this._angularLogService.log(`[${this.id}] Received INVITE`);
                ...
                if (this.session) {
                    this._angularLogService.warn(`[${this.id}] Session already in progress, rejecting INVITE...`);
                    invitation
                        .reject()
                        .then(() => {
                        this._angularLogService.log(`[${this.id}] Rejected INVITE`);
                    })
                        .catch((error) => {
                        this._angularLogService.error(`[${this.id}] Failed to reject INVITE`);
                        this._angularLogService.error(error.toString());
                    });
                    return;
                }
                ...
                if (this.delegate && this.delegate.onCallReceived) {
                    ...
                    this._angularLogService.warn(`[${this.id}] No handler available, rejecting INVITE...`);
                    invitation
                        .reject()
                        .then(() => {
                        this._angularLogService.log(`[${this.id}] Rejected INVITE`);
        ...
        this._angularLogService = this.userAgent.getLogger("sip.SimpleUser");
        // Monitor network connectivity and attempt reconnection when we come online
        window.addEventListener("online", () => {
            this._angularLogService.log(`[${this.id}] Online`);
            this.attemptReconnection();
        });
    }
    /**
     ...
     */
    connect() {
        this._angularLogService.log(`[${this.id}] Connecting UserAgent...`);
     ...
     */
    disconnect() {
        this._angularLogService.log(`[${this.id}] Disconnecting UserAgent...`);
        this.connectRequested = false;
    ...
    register(registererOptions, registererRegisterOptions) {
        this._angularLogService.log(`[${this.id}] Registering UserAgent...`);
    ...
    unregister(registererUnregisterOptions) {
        this._angularLogService.log(`[${this.id}] Unregistering UserAgent...`);
        this.registerRequested = false;
        if (!this.registerer) {
    ...
    call(destination, inviterOptions, inviterInviteOptions) {
        this._angularLogService.log(`[${this.id}] Beginning Session...`);
        if (this.session) {
     ...
     */
    hangup() {
        this._angularLogService.log(`[${this.id}] Hangup...`);
    ...
    answer(invitationAcceptOptions) {
        this._angularLogService.log(`[${this.id}] Accepting Invitation...`);
        if (!this.session) {
     ...
     */
    decline() {
        this._angularLogService.log(`[${this.id}] rejecting Invitation...`);
        if (!this.session) {
            return Promise.reject(new Error("Session does not exist."));
        }
        if (!(this.session instanceof Invitation)) {
     ...
     */
    hold() {
        this._angularLogService.log(`[${this.id}] holding session...`);
     ...
     */
    unhold() {
        this._angularLogService.log(`[${this.id}] unholding session...`);
     ...
     */
    mute() {
        this._angularLogService.log(`[${this.id}] disabling media tracks...`);
        this.setMute(true);
    }
    /**
     * Unmute call.
     ...
     */
    unmute() {
        this._angularLogService.log(`[${this.id}] enabling media tracks...`);
        this.setMute(false);
    }
    /**
     * Mute state.
     ...
     */
    sendDTMF(tone) {
        this._angularLogService.log(`[${this.id}] sending DTMF...`);
        ...
        this._angularLogService.log(`[${this.id}] Sending DTMF tone: ${tone}`);
        const dtmf = tone;
        const duration = 2000;
        const body = {
    ...
    message(destination, message) {
        this._angularLogService.log(`[${this.id}] sending message...`);
        const target = UserAgent.makeURI(destination);
        if (!target) {
            return Promise.reject(new Error(`Failed to create a valid URI from "${destination}"`));
        }
    ...
    attemptReconnection(reconnectionAttempt = 1) {
        ...
        if (!this.connectRequested) {
            this._angularLogService.log(`[${this.id}] Reconnection not currently desired`);
            return; // If intentionally disconnected, don't reconnect.
        }
        if (this.attemptingReconnection) {
            this._angularLogService.log(`[${this.id}] Reconnection attempt already in progress`);
        }
        if (reconnectionAttempt > reconnectionAttempts) {
            this._angularLogService.log(`[${this.id}] Reconnection maximum attempts reached`);
            return;
        }
        if (reconnectionAttempt === 1) {
            this._angularLogService.log(`[${this.id}] Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - trying`);
        }
        else {
            this._angularLogService.log(`[${this.id}] Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - trying in ${reconnectionDelay} seconds`);
        }
        this.attemptingReconnection = true;
        setTimeout(() => {
            if (!this.connectRequested) {
                this._angularLogService.log(`[${this.id}] Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - aborted`);
                this.attemptingReconnection = false;
                return; // If intentionally disconnected, don't reconnect.
            }
            this.userAgent
                .reconnect()
                .then(() => {
                this._angularLogService.log(`[${this.id}] Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - succeeded`);
                this.attemptingReconnection = false;
            })
                .catch((error) => {
                this._angularLogService.log(`[${this.id}] Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - failed`);
    ...
    initSession(session, referralInviterOptions) {
        ...
        this.session.stateChange.addListener((state) => {
            ...
            this._angularLogService.log(`[${this.id}] session state changed to ${state}`);
            switch (state) {
    ...
    sendInvite(inviter, inviterOptions, inviterInviteOptions) {
        ...
        return inviter.invite(inviterInviteOptions).then(() => {
            this._angularLogService.log(`[${this.id}] sent INVITE`);
        });
    }
    /**
    ...
    setHold(hold) {
        ...
        const options = {
            requestDelegate: {
                ...
                onReject: () => {
                    this._angularLogService.warn(`[${this.id}] re-invite request was rejected`);
                    this.enableReceiverTracks(!this.held);
                    this.enableSenderTracks(!this.held && !this.muted);
                    if (this.delegate && this.delegate.onCallHold) {
                        this.delegate.onCallHold(this.held);
        ...
        return this.session
            ...
            if (error instanceof RequestPendingError) {
                this._angularLogService.error(`[${this.id}] A hold request is already in progress.`);
            }
            throw error;
        });
    }
    ...
    setMute(mute) {
        if (!this.session) {
            this._angularLogService.warn(`[${this.id}] A session is required to enabled/disable media tracks`);
            return;
        }
        if (this.session.state !== SessionState.Established) {
            this._angularLogService.warn(`[${this.id}] An established session is required to enable/disable media tracks`);
            return;
        }
        this.muted = mute;
        this.enableSenderTracks(!this.held && !this.muted);
    ...
    setupLocalMedia() {
        ...
        if (mediaElement) {
            ...
            mediaElement.play().catch((error) => {
                this._angularLogService.error(`[${this.id}] Failed to play local media`);
                this._angularLogService.error(error.message);
            });
        }
    }
    ...
    setupRemoteMedia() {
        ...
        if (mediaElement) {
            ...
            mediaElement.play().catch((error) => {
                this._angularLogService.error(`[${this.id}] Failed to play remote media`);
                this._angularLogService.error(error.message);
            });
            remoteStream.onaddtrack = () => {
                this._angularLogService.log(`[${this.id}] Remote media onaddtrack`);
     ...
     */
    terminate() {
        this._angularLogService.log(`[${this.id}] Terminating...`);
        if (!this.session) {
            return Promise.reject(new Error("Session does not exist."));
        }
        switch (this.session.state) {
            case SessionState.Initial:
                if (this.session instanceof Inviter) {
                    return this.session.cancel().then(() => {
                        this._angularLogService.log(`[${this.id}] Inviter never sent INVITE (canceled)`);
                    });
                }
                else if (this.session instanceof Invitation) {
                    return this.session.reject().then(() => {
                        this._angularLogService.log(`[${this.id}] Invitation rejected (sent 480)`);
                    });
                }
                else {
            ...
            case SessionState.Establishing:
                if (this.session instanceof Inviter) {
                    return this.session.cancel().then(() => {
                        this._angularLogService.log(`[${this.id}] Inviter canceled (sent CANCEL)`);
                    });
                }
                else if (this.session instanceof Invitation) {
                    return this.session.reject().then(() => {
            ...
            case SessionState.Established:
                return this.session.bye().then(() => {
                    this._angularLogService.log(`[${this.id}] Session ended (sent BYE)`);
                });
            case SessionState.Terminating:
                break;
            case SessionState.Terminated:
        ...
        this._angularLogService.log(`[${this.id}] Terminating in state ${this.session.state}, no action taken`);
        return Promise.resolve();
    }
}
