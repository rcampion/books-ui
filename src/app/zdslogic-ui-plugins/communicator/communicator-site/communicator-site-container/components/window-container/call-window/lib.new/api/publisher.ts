...
export class Publisher {
  ...
  public constructor(userAgent: UserAgent, targetURI: URI, eventType: string, options: PublisherOptions = {}) {
    ...
    this._angularLogService = userAgent.getLogger("sip.Publisher");
  ...
  public dispose(): Promise<void> {
    ...
    this.disposed = true;
    this._angularLogService.log(`Publisher ${this.id} in state ${this.state} is being disposed`);
    ...
    switch (true) {
      ...
      case /^2[0-9]{2}$/.test(statusCode.toString()):
        ...
        if (response.hasHeader("SIP-ETag")) {
          ...
          this._angularLogService.warn("SIP-ETag header missing in a 200-class response to PUBLISH");
        }
        ...
        if (response.hasHeader("Expires")) {
          ...
          if (typeof expires === "number" && expires >= 0 && expires <= this.pubRequestExpires) {
            ...
            this._angularLogService.warn("Bad Expires header in a 200-class response to PUBLISH");
          }
        } else {
          this._angularLogService.warn("Expires header missing in a 200-class response to PUBLISH");
        }
      ...
      case /^412$/.test(statusCode.toString()):
        ...
        if (this.pubRequestEtag !== undefined && this.pubRequestExpires !== 0) {
          this._angularLogService.warn("412 response to PUBLISH, recovering");
          this.pubRequestEtag = undefined;
          ...
          this._angularLogService.warn("412 response to PUBLISH, recovery failed");
          this.pubRequestExpires = 0;
          this.stateTransition(PublisherState.Unpublished);
      ...
      case /^423$/.test(statusCode.toString()):
        ...
        if (this.pubRequestExpires !== 0 && response.hasHeader("Min-Expires")) {
          ...
          if (typeof minExpires === "number" || minExpires > this.pubRequestExpires) {
            this._angularLogService.warn("423 code in response to PUBLISH, adjusting the Expires value and trying to recover");
            ...
            this._angularLogService.warn("Bad 423 response Min-Expires header received for PUBLISH");
            this.pubRequestExpires = 0;
            this.stateTransition(PublisherState.Unpublished);
            this.stateTransition(PublisherState.Terminated);
          }
        } else {
          this._angularLogService.warn("423 response to PUBLISH, recovery failed");
          this.pubRequestExpires = 0;
          this.stateTransition(PublisherState.Unpublished);
          this.stateTransition(PublisherState.Terminated);
        }
  ...
  private stateTransition(newState: PublisherState): void {
    ...
    this._angularLogService.log(`Publication transitioned to state ${this._state}`);
