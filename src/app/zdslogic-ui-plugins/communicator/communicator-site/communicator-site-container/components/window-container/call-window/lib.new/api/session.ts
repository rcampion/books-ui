...
export abstract class Session {
  ...
  public dispose(): Promise<void> {
    this._angularLogService.log(`Session ${this.id} in state ${this._state} is being disposed`);
  ...
  public bye(options: SessionByeOptions = {}): Promise<OutgoingByeRequest> {
    ...
    this._angularLogService.error(message);
  ...
  public invite(options: SessionInviteOptions = {}): Promise<OutgoingInviteRequest> {
    this._angularLogService.log("Session.invite");
    if (this.state !== SessionState.Established) {
      return Promise.reject(new Error(`Invalid session state ${this.state}`));
    }
    ...
    const delegate: OutgoingInviteRequestDelegate = {
      onAccept: (response): void => {
        ...
        if (!body) {
          ...
          this._angularLogService.error("Received 2xx response to re-INVITE without a session description");
        ...
        if (options.withoutSdp) {
          ...
          this.setOfferAndGetAnswer(body, answerOptions)
            ...
            .catch((error: Error) => {
              // No way to recover, so terminate session and mark as failed.
              this._angularLogService.error("Failed to handle offer in 2xx response to re-INVITE");
              this._angularLogService.error(error.message);
          ...
          this.setAnswer(body, answerOptions)
            ...
            .catch((error: Error) => {
              // No way to recover, so terminate session and mark as failed.
              this._angularLogService.error("Failed to handle answer in 2xx response to re-INVITE");
              this._angularLogService.error(error.message);
      ...
      onReject: (response): void => {
        this._angularLogService.warn("Received a non-2xx response to re-INVITE");
        this.pendingReinvite = false;
        if (options.withoutSdp) {
          ...
          this.rollbackOffer()
            .catch((error: Error) => {
              // No way to recover, so terminate session and mark as failed.
              this._angularLogService.error("Failed to rollback offer on non-2xx response to re-INVITE");
              this._angularLogService.error(error.message);
              // A BYE should only be sent if session is not already terminated.
              // For example, a BYE may be sent/received while re-INVITE is outstanding.
              ...
              if (this.state !== SessionState.Terminated) {
                ...
                this._dialog.bye(undefined, { extraHeaders });
                this.stateTransition(SessionState.Terminated);
              }
            })
            .then(() => {
    ...
    if (options.withoutSdp) {
      ...
      return Promise.resolve(this._dialog.invite(delegate, requestOptions));
    }
    ...
    return this.getOffer(offerOptions)
      .then((offerBody) => {
        ...
        return this._dialog.invite(delegate, requestOptions);
      })
      .catch((error: Error) => {
        this._angularLogService.error(error.message);
        this._angularLogService.error("Failed to send re-INVITE");
        this.pendingReinvite = false;
        throw error;
      });
  }
  ...
  public _info(delegate?: OutgoingRequestDelegate, options?: RequestOptions): Promise<OutgoingInfoRequest> {
    ...
    return Promise.resolve(this._dialog.info(delegate, options));
  }
  ...
  public _message(delegate?: OutgoingRequestDelegate, options?: RequestOptions): Promise<OutgoingMessageRequest> {
    ...
    return Promise.resolve(this._dialog.message(delegate, options));
  }
  ...
  public _refer(
    ...
    return Promise.resolve(this._dialog.refer(delegate, options));
  }
   ...
   * which is not necessarily `this._dialog`. And, accordingly, the
  ...
  protected ackAndBye(response: AckableIncomingResponseWithSession, statusCode?: number, reasonPhrase?: string): void {
    ...
    // Using the dialog session associate with the response (which might not be this._dialog)
  ...
  protected onAckRequest(request: IncomingAckRequest): Promise<void> {
    this._angularLogService.log("Session.onAckRequest");
    if (this.state !== SessionState.Established && this.state !== SessionState.Terminating) {
      this._angularLogService.error(`ACK received while in state ${this.state}, dropping request`);
      return Promise.resolve();
    }
    ...
    switch (dialog.signalingState) {
      case SignalingState.Initial: {
        ...
        this._angularLogService.error(`Invalid signaling state ${dialog.signalingState}.`);
  ...
  protected onByeRequest(request: IncomingByeRequest): void {
    this._angularLogService.log("Session.onByeRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`BYE received while in state ${this.state}, dropping request`);
      return;
    }
  ...
  protected onInfoRequest(request: IncomingInfoRequest): void {
    this._angularLogService.log("Session.onInfoRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`INFO received while in state ${this.state}, dropping request`);
      return;
    }
  ...
  protected onInviteRequest(request: IncomingInviteRequest): void {
    this._angularLogService.log("Session.onInviteRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`INVITE received while in state ${this.state}, dropping request`);
      return;
    }
    ...
    this.generateResponseOfferAnswerInDialog(options)
      ...
      .catch((error: Error) => {
        this._angularLogService.error(error.message);
        this._angularLogService.error("Failed to handle to re-INVITE request");
        if (!this._dialog) {
          throw new Error("Dialog undefined.");
        }
        this._angularLogService.error(this._dialog.signalingState);
        // If we don't have a local/remote offer...
        if (this._dialog.signalingState === SignalingState.Stable) {
        ...
        this.rollbackOffer()
          ...
          .catch((errorRollback: Error) => {
            // No way to recover, so terminate session and mark as failed.
            this._angularLogService.error(errorRollback.message);
            this._angularLogService.error("Failed to rollback offer on re-INVITE request");
            const outgoingResponse = request.reject({ statusCode: 488 }); // Not Acceptable Here
            // A BYE should only be sent if session is not already terminated.
            // For example, a BYE may be sent/received while re-INVITE is outstanding.
            // Note that the ACK was already sent by the transaction, so just need to send BYE.
  ...
  protected onMessageRequest(request: IncomingMessageRequest): void {
    this._angularLogService.log("Session.onMessageRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`MESSAGE received while in state ${this.state}, dropping request`);
      return;
    }
  ...
  protected onNotifyRequest(request: IncomingNotifyRequest): void {
    this._angularLogService.log("Session.onNotifyRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`NOTIFY received while in state ${this.state}, dropping request`);
      return;
    }
  ...
  protected onPrackRequest(request: IncomingPrackRequest): void {
    this._angularLogService.log("Session.onPrackRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`PRACK received while in state ${this.state}, dropping request`);
      return;
    }
  ...
  protected onReferRequest(request: IncomingReferRequest): void {
    this._angularLogService.log("Session.onReferRequest");
    if (this.state !== SessionState.Established) {
      this._angularLogService.error(`REFER received while in state ${this.state}, dropping request`);
      return;
    }
    ...
    if (!request.message.hasHeader("refer-to")) {
      this._angularLogService.warn("Invalid REFER packet. A refer-to header is required. Rejecting.");
      request.reject();
      return;
    }
    ...
    if (this.delegate && this.delegate.onRefer) {
      ...
      this._angularLogService.log("No delegate available to handle REFER, automatically accepting and following.");
      referral
        .accept()
  ...
  protected generateResponseOfferAnswerInDialog(options: {
    ...
    switch (this._dialog.signalingState) {
      case SignalingState.Initial:
        return this.getOffer(options);
      case SignalingState.HaveLocalOffer:
      ...
      case SignalingState.Stable:
        ...
        return this.getOffer(options);
      case SignalingState.Closed:
        throw new Error(`Invalid signaling state ${this._dialog.signalingState}.`);
      default:
        throw new Error(`Invalid signaling state ${this._dialog.signalingState}.`);
    }
  }
    ...
    try {
      return sdh
        ...
        .catch((error: unknown) => {
          ...
          this._angularLogService.error("Session.getOffer: SDH getDescription rejected...");
          const e = error instanceof Error ? error : new Error("Session.getOffer unknown error.");
          this._angularLogService.error(e.message);
          throw e;
        });
    } catch (error) {
      // don't trust SDH to throw an Error
      this._angularLogService.error("Session.getOffer: SDH getDescription threw...");
  ...
  protected rollbackOffer(): Promise<void> {
    ...
    try {
      return sdh.rollbackDescription().catch((error: unknown) => {
        // don't trust SDH to reject with Error
        this._angularLogService.error("Session.rollbackOffer: SDH rollbackDescription rejected...");
      ...
      // don't trust SDH to throw an Error
      this._angularLogService.error("Session.rollbackOffer: SDH rollbackDescription threw...");
      const e = error instanceof Error ? error : new Error(error);
      this._angularLogService.error(e.message);
      return Promise.reject(e);
    }
  ...
  protected setAnswer(
    ...
    try {
      ...
      this._angularLogService.error("Session.setAnswer: SDH hasDescription threw...");
      const e = error instanceof Error ? error : new Error(error);
      this._angularLogService.error(e.message);
      return Promise.reject(e);
    }
    try {
      return sdh.setDescription(answer.content, sdhOptions, sdhModifiers).catch((error: unknown) => {
        // don't trust SDH to reject with Error
        this._angularLogService.error("Session.setAnswer: SDH setDescription rejected...");
      ...
      // don't trust SDH to throw an Error
      this._angularLogService.error("Session.setAnswer: SDH setDescription threw...");
      const e = error instanceof Error ? error : new Error(error);
      this._angularLogService.error(e.message);
      return Promise.reject(e);
    }
  ...
  protected setOfferAndGetAnswer(
    ...
    try {
      ...
      this._angularLogService.error("Session.setOfferAndGetAnswer: SDH hasDescription threw...");
      const e = error instanceof Error ? error : new Error(error);
      this._angularLogService.error(e.message);
      return Promise.reject(e);
    }
    try {
      return sdh
        ...
        .catch((error: unknown) => {
          // don't trust SDH to reject with Error
          this._angularLogService.error("Session.setOfferAndGetAnswer: SDH setDescription or getDescription rejected...");
      ...
      // don't trust SDH to throw an Error
      this._angularLogService.error("Session.setOfferAndGetAnswer: SDH setDescription or getDescription threw...");
      const e = error instanceof Error ? error : new Error(error);
      this._angularLogService.error(e.message);
      return Promise.reject(e);
    }
  ...
  protected stateTransition(newState: SessionState): void {
    ...
    this._angularLogService.log(`Session ${this.id} transitioned to state ${this._state}`);
