...
export class EMailSentUserFilesService {
	...
	findFilesWithSortAndFilter(
		...
		let apiUrl = this.createCompleteRoute('my-sent-emails', environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('my-sent-emails/search', environment.apiUrl);
	...
	getFile(id: string): any {
		...
		const route = `my-sent-emails/download/${id}`;
