...
export class LoggerFactory {
  ...
  constructor() {
    this._angularLogService = this.getLogger("sip:loggerfactory");
  }
  ...
  set level(newLevel: Levels) {
    if (newLevel >= 0 && newLevel <= 3) {
      ...
      this._angularLogService.error("invalid 'level' parameter value: " + JSON.stringify(newLevel));
    }
  }
  ...
  set connector(
    ...
    if (!value) {
      ...
      this._angularLogService.error("invalid 'connector' parameter value: " + JSON.stringify(value));
    }
  }
  ...
  public getLogger(category: string, label?: string): Logger {
    ...
    } else if (this._angularLogServices[category]) {
      return this._angularLogServices[category];
    } else {
      const logger = new Logger(this, category);
      this._angularLogServices[category] = logger;
      return logger;
    }
  }
