...
export class Inviter extends Session {
    ...
    constructor(userAgent, targetURI, options = {}) {
        ...
        this._angularLogService = userAgent.getLogger("sip.Inviter");
        // Early media
     ...
     */
    cancel(options = {}) {
        this._angularLogService.log("Inviter.cancel");
        // validate state
        ...
        if (this.outgoingInviteRequest) {
            ...
            this._angularLogService.warn("Canceled session before INVITE was sent");
     ...
     */
    invite(options = {}) {
        this._angularLogService.log("Inviter.invite");
        // validate state
        ...
        return this.getOffer(offerOptions)
            ...
            .catch((error) => {
            this._angularLogService.log(error.message);
            this.stateTransition(SessionState.Terminated);
            throw error;
        });
    }
    ...
    sendInvite(options = {}) {
        ...
        this.outgoingInviteRequest = this.userAgent.userAgentCore.invite(this.outgoingRequestMessage, {
            onAccept: (inviteResponse) => {
                ...
                if (this._dialog) {
                    this._angularLogService.log("Additional confirmed dialog, sending ACK and BYE");
                ...
                if (this.isCanceled) {
                    this._angularLogService.log("Canceled session accepted, sending ACK and BYE");
                    this.ackAndBye(inviteResponse);
                    this.stateTransition(SessionState.Terminated);
                    return;
                }
     ...
     */
    onAccept(inviteResponse) {
        this._angularLogService.log("Inviter.onAccept");
        // validate state
        if (this.state !== SessionState.Establishing) {
            this._angularLogService.error(`Accept received while in state ${this.state}, dropping response`);
        ...
        switch (session.signalingState) {
            case SignalingState.Initial:
                ...
                this._angularLogService.error("Received 2xx response to INVITE without a session description");
                    ...
                    if (this.earlyMediaDialog !== session) {
                        if (this.earlyMedia) {
                            const message = "You have set the 'earlyMedia' option to 'true' which requires that your INVITE requests " +
                                ...
                                "by this this._library. If you wish to use early media with this library you must configure those " +
    ...
    onProgress(inviteResponse) {
        var _a;
        this._angularLogService.log("Inviter.onProgress");
        // validate state
        if (this.state !== SessionState.Establishing) {
            this._angularLogService.error(`Progress received while in state ${this.state}, dropping response`);
            return Promise.reject(new Error(`Invalid session state ${this.state}`));
        }
        if (!this.outgoingInviteRequest) {
        ...
        switch (session.signalingState) {
            case SignalingState.Initial:
                ...
                if (responseReliable) {
                    ...
                    // provisional response.
                    //  https://tools.ietf.org/html/rfc3262#section-5
                    this._angularLogService.warn("First reliable provisional response received MUST contain an offer when INVITE does not contain an offer.");
            ...
            case SignalingState.HaveRemoteOffer:
                if (!responseReliable) {
                    ...
                    this._angularLogService.warn("Non-reliable provisional response MUST NOT contain an initial offer, discarding response.");
                    return Promise.resolve();
                }
                {
    ...
    onRedirect(inviteResponse) {
        this._angularLogService.log("Inviter.onRedirect");
        // validate state
        if (this.state !== SessionState.Establishing && this.state !== SessionState.Terminating) {
            this._angularLogService.error(`Redirect received while in state ${this.state}, dropping response`);
            return;
        }
        // transition state
        this.stateTransition(SessionState.Terminated);
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    onReject(inviteResponse) {
        this._angularLogService.log("Inviter.onReject");
        // validate state
        if (this.state !== SessionState.Establishing && this.state !== SessionState.Terminating) {
            this._angularLogService.error(`Reject received while in state ${this.state}, dropping response`);
            return;
        }
        // transition state
        this.stateTransition(SessionState.Terminated);
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    onTrying(inviteResponse) {
        this._angularLogService.log("Inviter.onTrying");
        // validate state
        if (this.state !== SessionState.Establishing) {
            this._angularLogService.error(`Trying received while in state ${this.state}, dropping response`);
            return;
        }
    }
}
