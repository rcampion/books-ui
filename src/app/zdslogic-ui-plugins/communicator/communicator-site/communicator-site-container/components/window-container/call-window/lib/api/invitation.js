...
export class Invitation extends Session {
    ...
    constructor(userAgent, incomingInviteRequest) {
        ...
        this._angularLogService = userAgent.getLogger("sip.Invitation");
     ...
     */
    accept(options = {}) {
        this._angularLogService.log("Invitation.accept");
        // validate state
        if (this.state !== SessionState.Initial) {
     ...
     */
    progress(options = {}) {
        this._angularLogService.log("Invitation.progress");
        // validate state
        if (this.state !== SessionState.Initial) {
            const error = new Error(`Invalid session state ${this.state}`);
            this._angularLogService.error(error.message);
        ...
        if (this.waitingForPrack) {
            this._angularLogService.warn("Unexpected call for progress while waiting for prack, ignoring");
            return Promise.resolve();
        }
     ...
     */
    reject(options = {}) {
        this._angularLogService.log("Invitation.reject");
        // validate state
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _onCancel(message) {
        this._angularLogService.log("Invitation._onCancel");
        // validate state
        if (this.state !== SessionState.Initial && this.state !== SessionState.Establishing) {
            this._angularLogService.error(`CANCEL received while in state ${this.state}, dropping request`);
            return;
        }
        // flag canceled
    ...
    handleResponseError(error) {
        ...
        if (error instanceof ContentTypeUnsupportedError) {
            this._angularLogService.error("A session description handler occurred while sending response (content type unsupported");
            statusCode = 415; // "Unsupported Media Type"
        }
        else if (error instanceof SessionDescriptionHandlerError) {
            this._angularLogService.error("A session description handler occurred while sending response");
        }
        else if (error instanceof SessionTerminatedError) {
            this._angularLogService.error("Session ended before response could be formulated and sent (while waiting for PRACK)");
        }
        else if (error instanceof TransactionStateError) {
            this._angularLogService.error("Session changed state before response could be formulated and sent");
        }
        ...
        if (this.state === SessionState.Initial || this.state === SessionState.Establishing) {
            try {
                ...
                this._angularLogService.error("An error occurred attempting to reject the request while handling another error");
        ...
        if (this.isCanceled) {
            this._angularLogService.warn("An error occurred while attempting to formulate and send a response to an incoming INVITE." +
     ...
     */
    onAckTimeout() {
        this._angularLogService.log("Invitation.onAckTimeout");
        if (!this._dialog) {
            throw new Error("Dialog undefined.");
        }
        this._angularLogService.log("No ACK received for an extended period of time, terminating session");
        this._dialog.bye();
        this.stateTransition(SessionState.Terminated);
    }
    /**
        ...
        return new Promise((resolve, reject) => {
            ...
            this.generateResponseOfferAnswer(this.incomingInviteRequest, responseOptions)
                ...
                const prackWaitTimeout = () => {
                    ...
                    this.waitingForPrack = false;
                    this._angularLogService.warn("No PRACK received, rejecting INVITE.");
                    // eslint-disable-next-line @typescript-eslint/no-use-before-define
                    clearTimeout(rel1xxRetransmissionTimer);
