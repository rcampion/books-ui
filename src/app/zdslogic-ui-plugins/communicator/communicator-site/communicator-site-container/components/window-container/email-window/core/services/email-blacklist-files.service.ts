...
export class EMailBlacklistFilesService {
	...
	findFilesWithSortAndNamesFilter(
		...
		let apiUrl = this.createCompleteRoute('blacklist', environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('blacklist/search', environment.apiUrl);
	...
	findFilesWithSortAndTextFilter(
		...
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute('blacklist/search/text', environment.apiUrl);
		const paramsx: any = { page: pageNumber, size: pageSize };
		if (sort != null) {
			paramsx.sort = sort.property + ',' + sort.direction;
		}
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('blacklist/search/text', environment.apiUrl);
