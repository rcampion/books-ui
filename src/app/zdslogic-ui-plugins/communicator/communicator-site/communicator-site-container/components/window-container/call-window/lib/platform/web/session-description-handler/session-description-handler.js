...
export class SessionDescriptionHandler {
     ...
     */
    close() {
        this._angularLogService.debug("SessionDescriptionHandler.close");
    ...
    getDescription(options, modifiers) {
        var _a, _b;
        this._angularLogService.debug("SessionDescriptionHandler.getDescription");
        if (this._peerConnection === undefined) {
        ...
        return this.getLocalMediaStream(options)
            ...
            .catch((error) => {
            this._angularLogService.error("SessionDescriptionHandler.getDescription failed - " + error);
            throw error;
        });
    }
    /**
    ...
    hasDescription(contentType) {
        this._angularLogService.debug("SessionDescriptionHandler.hasDescription");
    ...
    sendDtmf(tones, options) {
        this._angularLogService.debug("SessionDescriptionHandler.sendDtmf");
        if (this._peerConnection === undefined) {
            this._angularLogService.error("SessionDescriptionHandler.sendDtmf failed - peer connection closed");
            return false;
        }
        const senders = this._peerConnection.getSenders();
        if (senders.length === 0) {
            this._angularLogService.error("SessionDescriptionHandler.sendDtmf failed - no senders");
            return false;
        }
        const dtmfSender = senders[0].dtmf;
        if (!dtmfSender) {
            this._angularLogService.error("SessionDescriptionHandler.sendDtmf failed - no DTMF sender");
            return false;
        }
        ...
        try {
            ...
            this._angularLogService.error(e);
            return false;
        }
        this._angularLogService.log("SessionDescriptionHandler.sendDtmf sent via RTP: " + tones.toString());
        return true;
    }
    /**
    ...
    setDescription(sdp, options, modifiers) {
        this._angularLogService.debug("SessionDescriptionHandler.setDescription");
        if (this._peerConnection === undefined) {
            return Promise.reject(new Error("Peer connection closed."));
        }
        // Callback on data channel creation
        ...
        return this.getLocalMediaStream(options)
            ...
            .catch((error) => {
            this._angularLogService.error("SessionDescriptionHandler.setDescription failed - " + error);
            throw error;
        });
    }
    /**
    ...
    applyModifiers(sdp, modifiers) {
        this._angularLogService.debug("SessionDescriptionHandler.applyModifiers");
        ...
        return modifiers
            ...
            .then((modified) => {
            this._angularLogService.debug("SessionDescriptionHandler.applyModifiers - modified sdp");
    ...
    createDataChannel(options) {
        ...
        switch (this._peerConnection.signalingState) {
            case "stable":
                ...
                this._angularLogService.debug("SessionDescriptionHandler.createDataChannel - creating data channel");
                try {
    ...
    createLocalOfferOrAnswer(options) {
        ...
        switch (this._peerConnection.signalingState) {
            case "stable":
                ...
                this._angularLogService.debug("SessionDescriptionHandler.createLocalOfferOrAnswer - creating SDP offer");
            ...
            case "have-remote-offer":
                ...
                this._angularLogService.debug("SessionDescriptionHandler.createLocalOfferOrAnswer - creating SDP answer");
    ...
    getLocalMediaStream(options) {
        this._angularLogService.debug("SessionDescriptionHandler.getLocalMediaStream");
        if (this._peerConnection === undefined) {
            return Promise.reject(new Error("Peer connection closed."));
        }
    ...
    setLocalMediaStream(stream) {
        this._angularLogService.debug("SessionDescriptionHandler.setLocalMediaStream");
        ...
        const updateTrack = (newTrack) => {
            ...
            if (sender) {
                trackUpdates.push(new Promise((resolve) => {
                    this._angularLogService.debug(`SessionDescriptionHandler.setLocalMediaStream - replacing sender ${kind} track`);
                    resolve();
                }).then(() => sender
                    ...
                    .catch((error) => {
                    this._angularLogService.error(`SessionDescriptionHandler.setLocalMediaStream - failed to replace sender ${kind} track`);
                    throw error;
                })));
            }
            else {
                trackUpdates.push(new Promise((resolve) => {
                    this._angularLogService.debug(`SessionDescriptionHandler.setLocalMediaStream - adding sender ${kind} track`);
                    resolve();
                }).then(() => {
                    ...
                    try {
                        ...
                        this._angularLogService.error(`SessionDescriptionHandler.setLocalMediaStream - failed to add sender ${kind} track`);
                        throw error;
                    }
                    localStream.addTrack(newTrack);
                    SessionDescriptionHandler.dispatchAddTrackEvent(localStream, newTrack);
    ...
    getLocalSessionDescription() {
        this._angularLogService.debug("SessionDescriptionHandler.getLocalSessionDescription");
        if (this._peerConnection === undefined) {
            return Promise.reject(new Error("Peer connection closed."));
        }
    ...
    setLocalSessionDescription(sessionDescription) {
        this._angularLogService.debug("SessionDescriptionHandler.setLocalSessionDescription");
        if (this._peerConnection === undefined) {
            return Promise.reject(new Error("Peer connection closed."));
        }
    ...
    setRemoteSessionDescription(sessionDescription) {
        this._angularLogService.debug("SessionDescriptionHandler.setRemoteSessionDescription");
        if (this._peerConnection === undefined) {
            return Promise.reject(new Error("Peer connection closed."));
        }
        ...
        if (!sdp) {
            this._angularLogService.error("SessionDescriptionHandler.setRemoteSessionDescription failed - cannot set null sdp");
    ...
    setRemoteTrack(track) {
        this._angularLogService.debug("SessionDescriptionHandler.setRemoteTrack");
        const remoteStream = this._remoteMediaStream;
        if (remoteStream.getTrackById(track.id)) {
            this._angularLogService.debug(`SessionDescriptionHandler.setRemoteTrack - have remote ${track.kind} track`);
        }
        else if (track.kind === "audio") {
            this._angularLogService.debug(`SessionDescriptionHandler.setRemoteTrack - adding remote ${track.kind} track`);
    ...
    updateDirection(options) {
        ...
        switch (this._peerConnection.signalingState) {
            case "stable":
                // if we are stable, assume we are creating a local offer
                this._angularLogService.debug("SessionDescriptionHandler.updateDirection - setting offer direction");
                {
            ...
            case "have-remote-offer":
                // if we have a remote offer, assume we are creating a local answer
                this._angularLogService.debug("SessionDescriptionHandler.updateDirection - setting answer direction");
    ...
    iceGatheringComplete() {
        this._angularLogService.debug("SessionDescriptionHandler.iceGatheringComplete");
        // clear timer if need be
        if (this.iceGatheringCompleteTimeoutId !== undefined) {
            this._angularLogService.debug("SessionDescriptionHandler.iceGatheringComplete - clearing timeout");
        ...
        if (this.iceGatheringCompletePromise !== undefined) {
            this._angularLogService.debug("SessionDescriptionHandler.iceGatheringComplete - resolving promise");
    ...
    waitForIceGatheringComplete(restart = false, timeout = 0) {
        this._angularLogService.debug("SessionDescriptionHandler.waitForIceGatheringToComplete");
        if (this._peerConnection === undefined) {
        ...
        if (!restart && this._peerConnection.iceGatheringState === "complete") {
            this._angularLogService.debug("SessionDescriptionHandler.waitForIceGatheringToComplete - already complete");
            return Promise.resolve();
        }
        // only one may be waiting, reject any prior
        if (this.iceGatheringCompletePromise !== undefined) {
            this._angularLogService.debug("SessionDescriptionHandler.waitForIceGatheringToComplete - rejecting prior waiting promise");
        ...
        this.iceGatheringCompletePromise = new Promise((resolve, reject) => {
            ...
            if (timeout > 0) {
                this._angularLogService.debug("SessionDescriptionHandler.waitForIceGatheringToComplete - timeout in " + timeout);
                this.iceGatheringCompleteTimeoutId = setTimeout(() => {
                    this._angularLogService.debug("SessionDescriptionHandler.waitForIceGatheringToComplete - timeout");
    ...
    initPeerConnectionEventHandlers() {
        this._angularLogService.debug("SessionDescriptionHandler.initPeerConnectionEventHandlers");
        if (!this._peerConnection)
            throw new Error("Peer connection undefined.");
        ...
        peerConnection.onconnectionstatechange = (event) => {
            ...
            this._angularLogService.debug(`SessionDescriptionHandler.onconnectionstatechange ${newState}`);
        ...
        peerConnection.ondatachannel = (event) => {
            var _a;
            this._angularLogService.debug(`SessionDescriptionHandler.ondatachannel`);
        ...
        peerConnection.onicecandidate = (event) => {
            var _a;
            this._angularLogService.debug(`SessionDescriptionHandler.onicecandidate`);
        ...
        peerConnection.onicecandidateerror = (event) => {
            var _a;
            this._angularLogService.debug(`SessionDescriptionHandler.onicecandidateerror`);
        ...
        peerConnection.oniceconnectionstatechange = (event) => {
            ...
            this._angularLogService.debug(`SessionDescriptionHandler.oniceconnectionstatechange ${newState}`);
        ...
        peerConnection.onicegatheringstatechange = (event) => {
            ...
            this._angularLogService.debug(`SessionDescriptionHandler.onicegatheringstatechange ${newState}`);
        ...
        peerConnection.onnegotiationneeded = (event) => {
            var _a;
            this._angularLogService.debug(`SessionDescriptionHandler.onnegotiationneeded`);
        ...
        peerConnection.onsignalingstatechange = (event) => {
            ...
            this._angularLogService.debug(`SessionDescriptionHandler.onsignalingstatechange ${newState}`);
        ...
        peerConnection.onstatsended = (event) => {
            var _a;
            this._angularLogService.debug(`SessionDescriptionHandler.onstatsended`);
        ...
        peerConnection.ontrack = (event) => {
            ...
            this._angularLogService.debug(`SessionDescriptionHandler.ontrack ${kind} ${enabled}`);
