...
export class InviteUserAgentClient extends UserAgentClient implements OutgoingInviteRequest {
  ...
  protected onTransportError(error: TransportError): void {
    ...
    this._angularLogService.error("User agent client request transport error while sending ACK.");
  }
    ...
    switch (true) {
      ...
      case /^1[0-9]{2}$/.test(statusCode):
        ...
        //  https://tools.ietf.org/html/rfc3261#section-13.2.2.1
        {
          ...
          if (!message.toTag) {
            this._angularLogService.warn("Non-100 1xx INVITE response received without a to tag, dropping.");
            return;
          }
          ...
          if (!contact) {
            this._angularLogService.error("Non-100 1xx INVITE response received without a Contact header field, dropping.");
            return;
          }
          ...
          if (!earlyDialog.reliableSequenceGuard(message)) {
            this._angularLogService.warn("1xx INVITE reliable response received out of order or is a retransmission, dropping.");
            return;
          }
      ...
      case /^2[0-9]{2}$/.test(statusCode):
        ...
        //  https://tools.ietf.org/html/rfc3261#section-13.2.2.4
        {
          ...
          if (!message.toTag) {
            this._angularLogService.error("2xx INVITE response received without a to tag, dropping.");
            return;
          }
          ...
          const contact = message.parseHeader("contact");
          if (!contact) {
            this._angularLogService.error("2xx INVITE response received without a Contact header field, dropping.");
            return;
          }
