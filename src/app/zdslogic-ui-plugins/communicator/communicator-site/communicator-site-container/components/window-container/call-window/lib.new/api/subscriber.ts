...
export class Subscriber extends Subscription {
  ...
  public constructor(userAgent: UserAgent, targetURI: URI, eventType: string, options: SubscriberOptions = {}) {
    ...
    this._angularLogService = userAgent.getLogger("sip.Subscriber");
    if (options.body) {
      this.body = {
        body: options.body,
    ...
    if (options.expires === undefined) {
      ...
      this._angularLogService.warn(`Option "expires" must be a number. Using default of 3600.`);
      this.expires = 3600;
    } else {
  ...
  public dispose(): Promise<void> {
    ...
    this._angularLogService.log(`Subscription ${this.id} in state ${this.state} is being disposed`);
      ...
      switch (subscriptionState.state) {
        case "terminated":
          if (subscriptionState.reason) {
            this._angularLogService.log(`Terminated subscription with reason ${subscriptionState.reason}`);
...
class SubscriberRequest {
  ...
  public constructor(
    ...
    this._angularLogService = core.loggerFactory.getLogger("sip.Subscriber");
