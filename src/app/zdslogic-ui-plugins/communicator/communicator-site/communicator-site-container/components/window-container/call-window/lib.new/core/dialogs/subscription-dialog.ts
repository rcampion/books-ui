...
export class SubscriptionDialog extends Dialog implements Subscription {
  ...
  constructor(
    ...
    this._angularLogService = core.loggerFactory.getLogger("sip.subscribe-dialog");
    this._angularLogService.log(`SUBSCRIBE dialog ${this.id} constructed`);
  }
  ...
  public dispose(): void {
    ...
    this._angularLogService.log(`SUBSCRIBE dialog ${this.id} destroyed`);
  }
  ...
  public receiveRequest(message: IncomingRequestMessage): void {
    this._angularLogService.log(`SUBSCRIBE dialog ${this.id} received ${message.method} request`);

    // Request within a dialog out of sequence guard.
    //  https://tools.ietf.org/html/rfc3261#section-12.2.2
    if (!this.sequenceGuard(message)) {
      this._angularLogService.log(`SUBSCRIBE dialog ${this.id} rejected out of order ${message.method} request.`);
      return;
    }
    ...
    switch (message.method) {
      case C.NOTIFY:
        ...
        break;
      default:
        this._angularLogService.log(`SUBSCRIBE dialog ${this.id} received unimplemented ${message.method} request`);
  ...
  public subscribe(
    ...
    this._angularLogService.log(`SUBSCRIBE dialog ${this.id} sending SUBSCRIBE request`);
  ...
  private onNotify(message: IncomingRequestMessage): void {
    ...
    switch (state) {
      ...
      case "terminated":
        ...
        break;
      default:
        this._angularLogService.warn("Unrecognized subscription state.");
        break;
    }
  ...
  private stateTransition(newState: SubscriptionState, newExpires?: number): void {
    ...
    const invalidStateTransition = (): void => {
      this._angularLogService.warn(`Invalid subscription state transition from ${this.subscriptionState} to ${newState}`);
    };
   ...
   */
  private timerN(): void {
    this._angularLogService.warn(`Timer N expired for SUBSCRIBE dialog. Timed out waiting for NOTIFY.`);
