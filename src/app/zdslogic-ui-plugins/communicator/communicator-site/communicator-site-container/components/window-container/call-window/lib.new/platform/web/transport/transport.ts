...
export class Transport implements TransportDefinition {
  ...
  constructor(logger: Logger, options?: TransportOptions) {
    ...
    if (parsed === -1) {
      this._angularLogService.error(`Invalid WebSocket Server URL "${url}"`);
      throw new Error("Invalid WebSocket Server URL");
    }
    if (!["wss", "ws", "udp"].includes(parsed.scheme)) {
      this._angularLogService.error(`Invalid scheme in WebSocket Server URL "${url}"`);
  ...
  private _connect(): Promise<void> {
    this._angularLogService.log(`Connecting ${this.server}`);
    ...
    try {
      ...
      this._ws = undefined;
      this._angularLogService.error("WebSocket construction failed.");
      this._angularLogService.error(error);
  ...
  private _disconnect(): Promise<void> {
    this._angularLogService.log(`Disconnecting ${this.server}`);
    ...
    this.disconnectPromise = new Promise((resolve, reject) => {
      ...
      try {
        ...
        this._angularLogService.error("WebSocket close failed.");
        this._angularLogService.error(error);
        throw error;
      }
    });
  ...
  private _send(message: string): Promise<void> {
    if (this.configuration.traceSip === true) {
      this._angularLogService.log("Sending WebSocket message:\n\n" + message + "\n");
    }
  ...
  private onWebSocketClose(ev: CloseEvent, ws: WebSocket): void {
    ...
    if (error) {
      this._angularLogService.warn("WebSocket closed unexpectedly");
    }
    this._angularLogService.log(message);
  ...
  private onWebSocketError(ev: Event, ws: WebSocket): void {
    ...
    this._angularLogService.error("WebSocket error occurred.");
  }
  ...
  private onWebSocketMessage(ev: MessageEvent, ws: WebSocket): void {
    ...
    if (/^(\r\n)+$/.test(data)) {
      ...
      if (this.configuration.traceSip === true) {
        this._angularLogService.log("Received WebSocket message with CRLF Keep Alive response");
      }
      return;
    }

    if (!data) {
      this._angularLogService.warn("Received empty message, discarding...");
      return;
    }
    ...
    if (typeof data !== "string") {
      ...
      try {
        ...
        this._angularLogService.error(err);
        this._angularLogService.error("Received WebSocket binary message failed to be converted into string, message discarded");
        return;
      }
      if (this.configuration.traceSip === true) {
        this._angularLogService.log("Received WebSocket binary message:\n\n" + finishedData + "\n");
      }
    } else {
      // WebSocket text message.
      finishedData = data;
      if (this.configuration.traceSip === true) {
        this._angularLogService.log("Received WebSocket text message:\n\n" + finishedData + "\n");
      }
    }

    if (this.state !== TransportState.Connected) {
      this._angularLogService.warn("Received message while not connected, discarding...");
      return;
    }
    ...
    if (this.onMessage) {
      try {
        ...
        this._angularLogService.error("Exception thrown by onMessage callback");
  ...
  private onWebSocketOpen(ev: Event, ws: WebSocket): void {
    ...
    if (this._state === TransportState.Connecting) {
      this._angularLogService.log(`WebSocket opened ${this.server}`);
  ...
  private transitionState(newState: TransportState, error?: Error): void {
    ...
    this._angularLogService.log(`Transitioned from ${oldState} to ${this._state}`);
    ...
    if (newState === TransportState.Connected) {
      ...
      if (this.onConnect) {
        try {
          ...
          this._angularLogService.error(e);
          this._angularLogService.error("Exception thrown by onConnect callback");
          throw e; // rethrow unhandled exception
        }
      }
    }
    ...
    if (oldState === TransportState.Connected) {
      ...
      if (this.onDisconnect) {
        try {
          ...
          this._angularLogService.error(e);
          this._angularLogService.error("Exception thrown by onDisconnect callback");
          throw e; // rethrow unhandled exception
        }
      }
    }
