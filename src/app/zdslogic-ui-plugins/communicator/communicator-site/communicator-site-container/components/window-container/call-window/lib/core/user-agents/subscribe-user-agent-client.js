...
export class SubscribeUserAgentClient extends UserAgentClient {
    ...
    onNotify(uas) {
        ...
        if (!event || event !== this.subscriptionEvent) {
            this._angularLogService.warn(`Failed to parse event.`);
        ...
        if (!subscriptionState || !subscriptionState.state) {
            this._angularLogService.warn("Failed to parse subscription state.");
            uas.reject({ statusCode: 489 });
            return;
        }
        ...
        switch (state) {
            ...
            case "terminated":
                break;
            default:
                this._angularLogService.warn(`Invalid subscription state ${state}`);
                uas.reject({ statusCode: 489 });
                return;
        }
        ...
        if (state !== "terminated") {
            ...
            if (!contact) {
                this._angularLogService.warn("Failed to parse contact.");
                uas.reject({ statusCode: 489 });
                return;
            }
        }
        ...
        if (this.subscriptionState !== SubscriptionState.Terminated) {
            ...
            this._dialog = new SubscriptionDialog(this.subscriptionEvent, this.subscriptionExpires, this.subscriptionState, this.core, dialogState);
        }
        // Delegate.
    ...
    receiveResponse(message) {
        ...
        if (message.statusCode && message.statusCode >= 200 && message.statusCode < 300) {
            ...
            const expires = message.getHeader("Expires");
            if (!expires) {
                this._angularLogService.warn("Expires header missing in a 200-class response to SUBSCRIBE");
            }
            else {
                const subscriptionExpiresReceived = Number(expires);
                if (subscriptionExpiresReceived > this.subscriptionExpiresRequested) {
                    this._angularLogService.warn("Expires header in a 200-class response to SUBSCRIBE with a higher value than the one in the request");
                }
            ...
            if (this._dialog) {
                if (this._dialog.subscriptionExpires > this.subscriptionExpires) {
                    this._dialog.subscriptionExpires = this.subscriptionExpires;
                }
            }
        }
     ...
     */
    timerN() {
        this._angularLogService.warn(`Timer N expired for SUBSCRIBE user agent client. Timed out waiting for NOTIFY.`);
        this.waitNotifyStop();
