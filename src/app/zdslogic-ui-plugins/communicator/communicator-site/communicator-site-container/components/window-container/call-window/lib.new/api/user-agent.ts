...
export class UserAgent {
  ...
  public constructor(options: Partial<UserAgentOptions> = {}) {
    ...
    this._angularLogServiceFactory = new LoggerFactory();
    this._angularLogService = this._angularLogServiceFactory.getLogger("sip.UserAgent");
    this._angularLogServiceFactory.builtinEnabled = this.options.logBuiltinEnabled;
    this._angularLogServiceFactory.connector = this.options.logConnector as (
      level: string,
      category: string,
    ...
    switch (this.options.logLevel) {
      case "error":
        this._angularLogServiceFactory.level = Levels.error;
        break;
      case "warn":
        this._angularLogServiceFactory.level = Levels.warn;
        break;
      case "log":
        this._angularLogServiceFactory.level = Levels.log;
        break;
      case "debug":
        this._angularLogServiceFactory.level = Levels.debug;
        break;
      default:
        break;
    }
    ...
    if (this.options.logConfiguration) {
      this._angularLogService.log("Configuration:");
      Object.keys(this.options).forEach((key) => {
        ...
        switch (key) {
          ...
          case "sessionDescriptionHandlerFactory":
            this._angularLogService.log("· " + key + ": " + value);
            break;
          case "authorizationPassword":
            this._angularLogService.log("· " + key + ": " + "NOT SHOWN");
            break;
          case "transportConstructor":
            this._angularLogService.log("· " + key + ": " + value.name);
            break;
          default:
            this._angularLogService.log("· " + key + ": " + JSON.stringify(value));
        }
      });
    }
    ...
    if (this.options.transportOptions) {
      ...
      if (maxReconnectionAttemptsDeprecated !== undefined) {
        ...
        this._angularLogService.warn(deprecatedMessage);
      }
  ...
  public getLogger(category: string, label?: string): Logger {
    return this._angularLogServiceFactory.getLogger(category, label);
  }
  ...
  public getLoggerFactory(): LoggerFactory {
    return this._angularLogServiceFactory;
  }
  ...
  public start(): Promise<void> {
    if (this.state === UserAgentState.Started) {
      this._angularLogService.warn(`User agent already started`);
      return Promise.resolve();
    }
    this._angularLogService.log(`Starting ${this.configuration.uri}`);
  ...
  public async stop(): Promise<void> {
    if (this.state === UserAgentState.Stopped) {
      this._angularLogService.warn(`User agent already stopped`);
      return Promise.resolve();
    }
    this._angularLogService.log(`Stopping ${this.configuration.uri}`);
    ...
    this._angularLogService.log(`Dispose of registerers`);
    ...
    // Dispose of Sessions
    this._angularLogService.log(`Dispose of sessions`);
    ...
    this._angularLogService.log(`Dispose of subscriptions`);
    ...
    this._angularLogService.log(`Dispose of publishers`);
    ...
    this._angularLogService.log(`Dispose of transport`);
    ...
    this._angularLogService.log(`Dispose of core`);
  ...
  private attemptReconnection(reconnectionAttempt = 1): void {
    ...
    if (reconnectionAttempt > reconnectionAttempts) {
      this._angularLogService.log(`Maximum reconnection attempts reached`);
      return;
    }

    this._angularLogService.log(`Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - trying`);
    setTimeout(
      () => {
        this.reconnect()
          .then(() => {
            this._angularLogService.log(`Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - succeeded`);
          })
          .catch((error: Error) => {
            this._angularLogService.error(error.message);
            this._angularLogService.log(`Reconnection attempt ${reconnectionAttempt} of ${reconnectionAttempts} - failed`);
  ...
  private initCore(): UserAgentCore {
    ...
    const userAgentCoreConfiguration: UserAgentCoreConfiguration = {
      ...
      loggerFactory: this._angularLogServiceFactory,
    ...
    const userAgentCoreDelegate: UserAgentCoreDelegate = {
      onInvite: (incomingInviteRequest: IncomingInviteRequest): void => {
        ...
        incomingInviteRequest.delegate = {
          ...
          onTransportError: (error: TransportError): void => {
            ...
            this._angularLogService.error("A transport error has occurred while handling an incoming INVITE request.");
          }
        };
      ...
      onRefer: (incomingReferRequest: IncomingReferRequest): void => {
        this._angularLogService.warn("Received an out of dialog REFER request");
      ...
      onRegister: (incomingRegisterRequest: IncomingRegisterRequest): void => {
        this._angularLogService.warn("Received an out of dialog REGISTER request");
      ...
      onSubscribe: (incomingSubscribeRequest: IncomingSubscribeRequest): void => {
        this._angularLogService.warn("Received an out of dialog SUBSCRIBE request");
  ...
  private onTransportMessage(messageString: string): void {
    ...
    if (!message) {
      this._angularLogService.warn("Failed to parse incoming message. Dropping.");
      return;
    }

    if (this.state === UserAgentState.Stopped && message instanceof IncomingRequestMessage) {
      this._angularLogService.warn(`Received ${message.method} request while stopped. Dropping.`);
      return;
    }
    ...
    const hasMinimumHeaders = (): boolean => {
      ...
      for (const header of mandatoryHeaders) {
        if (!message.hasHeader(header)) {
          this._angularLogService.warn(`Missing mandatory header field : ${header}.`);
          return false;
        }
      }
      return true;
    ...
    if (message instanceof IncomingRequestMessage) {
      ...
      if (!hasMinimumHeaders()) {
        this._angularLogService.warn(`Request missing mandatory header field. Dropping.`);
        return;
      }
    ...
    if (message instanceof IncomingResponseMessage) {
      // This is port of SanityCheck.minimumHeaders().
      if (!hasMinimumHeaders()) {
        this._angularLogService.warn(`Response missing mandatory header field. Dropping.`);
        return;
      }
      ...
      if (message.getHeaders("via").length > 1) {
        this._angularLogService.warn("More than one Via header field present in the response. Dropping.");
        return;
      }
      ...
      if (message.via.host !== this.options.viaHost || message.via.port !== undefined) {
        this._angularLogService.warn("Via sent-by in the response does not match UA Via host value. Dropping.");
        return;
      }
      ...
      const contentLength: string | undefined = message.getHeader("content-length");
      if (contentLength && len < Number(contentLength)) {
        this._angularLogService.warn("Message body length is lower than the value in Content-Length header field. Dropping.");
        return;
      }
    }
  ...
  private transitionState(newState: UserAgentState, error?: Error): void {
    ...
    // Update state
    this._angularLogService.log(`Transitioned from ${this._state} to ${newState}`);
