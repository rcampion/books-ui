...
export class UserAgentClient {
    constructor(transactionConstructor, core, message, delegate) {
        ...
        this.stale = false;
        this._angularLogService = this._angularLogServiceFactory.getLogger("sip.user-agent-client");
        this.init();
    }
    dispose() {
    ...
    authenticationGuard(message, dialog) {
        ...
        if (!challenge) {
            this._angularLogService.warn(statusCode + " with wrong or missing challenge, cannot authenticate");
            return true;
        }
        // Avoid infinite authentications.
        if (this.challenged && (this.stale || challenge.stale !== true)) {
            this._angularLogService.warn(statusCode + " apparently in authentication loop, cannot authenticate");
            return true;
        }
        // Get credentials.
        if (!this.credentials) {
            ...
            if (!this.credentials) {
                this._angularLogService.warn("Unable to obtain credentials, cannot authenticate");
                return true;
            }
        }
     ...
     */
    onRequestTimeout() {
        this._angularLogService.warn("User agent client request timed out. Generating internal 408 Request Timeout.");
    ...
    onTransportError(error) {
        ...
        this._angularLogService.error("User agent client request transport error. Generating internal 503 Service Unavailable.");
        const message = new IncomingResponseMessage();
