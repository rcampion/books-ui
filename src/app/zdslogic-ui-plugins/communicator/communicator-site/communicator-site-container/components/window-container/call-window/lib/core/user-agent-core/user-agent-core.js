...
export class UserAgentCore {
    ...
    constructor(configuration, delegate = {}) {
        ...
        this._angularLogService = configuration.loggerFactory.getLogger("sip.user-agent-core");
    }
    /** Destructor. */
    dispose() {
        this.reset();
    ...
    replyStateless(message, options) {
        ...
        this.transport.send(response.message).catch((error) => {
            ...
            this._angularLogService.error(`Transport error occurred sending stateless reply to ${message.method} request.`);
        ...
        if (message.method === C.ACK) {
            if (uas && uas.transaction.state === TransactionState.Accepted) {
                if (uas instanceof InviteUserAgentServer) {
                    ...
                    this._angularLogService.warn(`Discarding out of dialog ACK after 2xx response sent on transaction ${transactionId}.`);
                    return;
                }
            }
        }
    ...
    receiveRequest(message) {
        ...
        if (!ruriMatches(this.configuration.aor) &&
            ...
            this._angularLogService.warn("Request-URI does not point to us.");
            if (message.method !== C.ACK) {
    ...
    receiveResponseFromTransport(message) {
        ...
        if (userAgentClient) {
            ...
            this._angularLogService.warn(`Discarding unmatched ${message.statusCode} response to ${message.method} ${userAgentClientId}.`);
        }
    }
}
