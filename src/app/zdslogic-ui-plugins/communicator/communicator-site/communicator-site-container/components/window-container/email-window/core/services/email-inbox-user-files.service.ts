...
export class EMailInboxUserFilesService {
	...
	findFilesWithSortAndFilter(
		...
		let apiUrl = this.createCompleteRoute('email-inbox/my-inbox-emails', environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('email-inbox/my-inbox-emails/search', environment.apiUrl);
	...
	getFile(id: string): any {
		...
		const route = `email-inbox/my-inbox-emails/download/${id}`;
