...
export class DigestAuthentication {
  ...
  constructor(
    ...
    this._angularLogService = loggerFactory.getLogger("sipjs.digestauthentication");
  ...
  public authenticate(request: OutgoingRequestMessage, challenge: any, body?: string): boolean {
    ...
    if (this.algorithm) {
      if (this.algorithm !== "MD5") {
        this._angularLogService.warn("challenge with Digest algorithm different than 'MD5', authentication aborted");
        return false;
      }
    } else {
      this.algorithm = "MD5";
    ...
    if (!this.realm) {
      this._angularLogService.warn("challenge without Digest realm, authentication aborted");
      return false;
    }

    if (!this.nonce) {
      this._angularLogService.warn("challenge without Digest nonce, authentication aborted");
      return false;
    }
    ...
    if (challenge.qop) {
      if (challenge.qop.indexOf("auth") > -1) {
        ...
        this._angularLogService.warn("challenge without Digest qop different than 'auth' or 'auth-int', authentication aborted");
        return false;
      }
    } else {
      this.qop = undefined;
