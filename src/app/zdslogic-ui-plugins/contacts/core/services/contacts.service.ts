...
export class ContactsService {
	...
	public createNoToken(route: string, body): any {
	...
	findContacts(
		...
		const apiUrl = this.createCompleteRoute('contacts', environment.apiUrl);
	...
	findContactsWithSortAndFilter(
		...
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute('contacts', environment.apiUrl);
