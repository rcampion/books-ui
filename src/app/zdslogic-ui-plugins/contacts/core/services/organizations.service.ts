...
export class OrganizationsService {
	...
	findOrganizationsWithSortAndFilter(

		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute('organization', environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('organization/search', environment.apiUrl);

			const organizationName = '\'*' + filter + '*\'';
			const organizationDescription = '\'*' + filter + '*\'';

			search = 'organizationName==' + organizationName + ' or ' + 'organizationDescription==' + organizationDescription;
	...
	findOrganizationMembersWithSortAndFilter(
		organizationId = 0,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {

		const id: number = organizationId;
		const buildApiUrl = 'organization/member/' + id;
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('contact/search', environment.apiUrl);
