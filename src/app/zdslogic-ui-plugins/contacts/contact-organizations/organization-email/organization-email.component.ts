...
@Component({
	selector: 'app-organization-email',
	templateUrl: './organization-email.component.html',
	styleUrls: ['./organization-email.component.scss']
})
export class OrganizationEMailComponent implements OnInit {
	...
	private getEMailDetails(): void {
		...
		let apiUrl = `contact/email/email/${this.emailId}`;

		this._contactsService.getData(apiUrl)
			.subscribe((result) => {
				this.email = result as EMail;

				apiUrl = `contact/${this.email.contactId}`;
	...
	private executeSendEMail(emailFormValue): void {
		...
		let apiUrl = 'contacts/organization/email/send';

		if (this.files !== undefined) {
			filedata = this.files[0];
			formData.append('file', filedata);
			apiUrl = 'contacts/organization/email/send/with/file';
		}
