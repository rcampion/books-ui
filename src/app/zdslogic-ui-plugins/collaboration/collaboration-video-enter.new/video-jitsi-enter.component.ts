...
export class VideoJitsiEnterComponent implements OnInit, AfterViewInit {
	...
	handleParticipantLeft = async (participant): Promise<void> => {
		//console.log("handleParticipantLeft", participant); // { id: "2baa184e" }
		const data = await this.getParticipants();
	};

	handleParticipantJoined = async (participant): Promise<void> => {
		//console.log("handleParticipantJoined", participant); // { id: "2baa184e", displayName: "Shanu Verma", formattedDisplayName: "Shanu Verma" }
		const data = await this.getParticipants();
	};

	handleVideoConferenceJoined = async (participant): Promise<void> => {
