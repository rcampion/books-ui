...
export class ProductAttributesService {
	...
	findProductAttributesWithSortAndFilter(
		id:number,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute(`products/product/productAttributes/${id}`, environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('productAttribute/search', environment.apiUrl);
