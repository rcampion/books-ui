...
export class ProductBundlesService {
	...
	findProductBundlesWithSortAndFilter(
		id: number,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute(`products/product/productBundles/${id}`, environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('productBundle/search', environment.apiUrl);
	...
	findProductBundleMembersWithSortAndFilter(
		productId = 0,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
