...
export class ProductSkuMediaMapsService {
	...
	findProductSkuMediaMapsWithSortAndFilter(
		id:number,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute(`products/products/sku/${id}`, environment.apiUrl);
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('products/sku/search', environment.apiUrl);
	...
	findSkuMembersWithSortAndFilter(
		productId = 0,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {

		const id: number = productId;
		const buildApiUrl = 'sku/member/' + id;
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('sku/search', environment.apiUrl);
