...
export class ProductSkusService {
	...
	findProductSkusByDefaultProductIdWithSortAndFilter(
		id: string,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute(`products/product/skus/${id}`, environment.apiUrl);
	...
	findProductSkusByAddlProductIdWithSortAndFilter(
		id: string,
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute(`products/product/skus/addl/${id}`, environment.apiUrl);
		const paramsx: any = { page: pageNumber, size: pageSize };
		if (sort != null) {
			paramsx.sort = sort.property + ',' + sort.direction;
		}
